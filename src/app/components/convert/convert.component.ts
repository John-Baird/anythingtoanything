        ...
        next: (res) => {
          ...
          if (res.kind === 'unsupported' || res.targets.length === 0) {
            ...
            this.errorMessage.set(
              "That file type isn't supported yet. Try an image (JPG, PNG, WebP, GIF, TIFF, AVIF), audio (MP3, WAV, FLAC, OGG, M4A), video (MP4, MOV, WebM, MKV), a document (TXT, MD, HTML, DOCX, PDF), or an archive (ZIP, 7Z, RAR, TAR, GZ).",
            );
            return;
          }
